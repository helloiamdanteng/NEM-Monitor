...
(re.compile(r”^(STAN[-*]?\d)”, re.I),           “Black Coal”),  # Stanwell alt
...
p = _Path(**file**).parent / “nem_units.json”
try:
...
def *parse_aemo(text: str, table_key: str) -> list[dict]:
...
key = f”{row[1].strip()}*{row[2].strip()}”.upper()
